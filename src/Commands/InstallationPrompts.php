...
final class InstallationPrompts
{
    public function collect(string $basePath, string $stepCatalogue, StepCatalogue $catalogue, callable $error): ?array
    {
        ...
        $siteId = text(label: __('Site ID'), default: $suggestedSiteId, required: true, validate: fn (string $value): ?string => $this->validSiteId($value) ? null : __('Use 1-32 lowercase letters or digits, with single hyphens between segments.'));
