...
[[ "$prerequisites_text" == *"php\${PHP_VERSION}-intl"* ]]
[[ "$prerequisites_text" == *'software-properties-common caddy'* ]]
[[ "$prerequisites_text" == *'for required_command in composer git curl jq unzip caddy'* ]]
[[ "$prerequisites_text" == *'apt-get install --reinstall -y'* ]]
...
[[ "$node_text" == *"apt-get install -y \"\${missing_packages[@]}\""* ]]
