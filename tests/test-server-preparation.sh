...
[[ "$bootstrap_text" == *'php artisan metator:provision --config=metator.production.php'* ]]
[[ "$bootstrap_text" != *'vendor/bin/dep deploy production'* ]]
deployer_login_text="$(<"$ROOT_DIR/stubs/scripts/steps/02-deployer-login.sh")"
[[ "$deployer_login_text" == *'if [[ -n "${CLIENT_PUBLIC_KEY:-}" ]]'* ]]
[[ "$deployer_login_text" == *'run provisioning with a TTY'* ]]
